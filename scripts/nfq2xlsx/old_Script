...
def get_list_of_data() -> tuple[list[Path], list[Path]]:
    (list_main, list_date) = (
        sorted(
            [
                f
                for f in Path(cp.TEMP_XLSX_MAIN_DIRPATH).glob("*.xlsx")
                if f.name[1:5] == cc.progress
            ]
        ),
        sorted(
            [
                f
                for f in Path(cp.TEMP_XLSX_DATE_DIRPATH).glob("*.xlsx")
                if f.name[1:5] == cc.progress
            ]
        ),
    )
...
def append_data(
    main_path: Path, date_ws: worksheet.Worksheet
...
def make_xlsx_shorter(xlsx_path: Path) -> None:
    ...
    serial_columns_temp = [
        list(col) for col in numpy_ws[:, : cc.REMOVE_COLUMN_FOR_XLSX2CSV[-1]].T
    ]
    specific_columns = [
        list(col) for col in numpy_ws[:, cc.REMOVE_COLUMN_FOR_XLSX2CSV[-1] :].T
    ]
    ...
    serial_columns = serial_columns_temp[cc.SUBJECTNAME_COLUMN][
        1 : cc.ACTUAL_RANGE_ROW["end"]
    ]
    serial_columns.insert(1, "決算発表日")
    serial_columns[0] = "決算発表月"

    new_numpy_ws_temp.append(
        serial_columns
    )  # append subject name column  # insert "決算発表月" column header
    ...
    for column in specific_columns:
        if (
            column[cc.KESSANTANSIN_DATE_ROW] is None
            and column[cc.YUUKAHOUKOKU_DATE_ROW] is None
        ):
            raise ValueError(
                f"No date found for {column_subjectname[0]} ({column_subjectcode[0]})."
            )
            ...
            for i in range(
                cc.YUUKAHOUKOKU_RANGE_ROW["start"], cc.YUUKAHOUKOKU_RANGE_ROW["end"]
            ):
            ...
            for i in range(
                cc.KESSANTANSIN_RANGE_ROW["start"], cc.KESSANTANSIN_RANGE_ROW["end"]
            ):
                new_column.append(column[i])
        ...
        # if different, first, extract the earlier date and related data
            ...
            new_column_KESSANTANSIN.append(
                column[cc.KESSANTANSIN_DATE_ROW]
            )  # append date
            new_column_YUUKAHOUKOKU.append(
                column[cc.YUUKAHOUKOKU_DATE_ROW]
            )  # append date
            ...
            if new_column_KESSANTANSIN[1] == "-":
                ...
                dt.strptime(column[cc.KESSANTANSIN_DATE_ROW], "%Y/%m/%d")
            ) < dt.timestamp(dt.strptime(column[cc.YUUKAHOUKOKU_DATE_ROW], "%Y/%m/%d")):
                new_numpy_ws_temp.append(new_column_KESSANTANSIN)
                new_numpy_ws_temp.append(new_column_YUUKAHOUKOKU)
            elif dt.timestamp(
                dt.strptime(column[cc.KESSANTANSIN_DATE_ROW], "%Y/%m/%d")
    ...
    pd.DataFrame(new_numpy_ws).to_excel(
        xlsx_path, index=False, header=False, sheet_name=inherited_ws_title
    )
...
def main():
    ...
    for head_data_pair in zip(list_main, list_date):
        ...
        make_xlsx_shorter(
            Path(cp.TEMP_XLSX_COMPLETE_DIRPATH)
            / (_helper.helper_get_ticker(main_ws) + ".xlsx")
        )
